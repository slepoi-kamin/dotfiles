...
2. **Local context** (`/Users/alex.ulyanov/dotfiles/.config.opencode/context/` in project root) — project-specific, highest priority
...
|---|---|---|
| **Project Intelligence** (tech stack, patterns, naming) | Local `/Users/alex.ulyanov/dotfiles/.config.opencode/context/project-intelligence/` | Project-specific, committed to git, shared with team |
...
**Example**: User installs OAC globally (core standards at `~/.config/opencode/context/core/`), then runs `/add-context` in a project (creates `/Users/alex.ulyanov/dotfiles/.config.opencode/context/project-intelligence/` locally). The agent loads both: core standards from global, project intelligence from local.
...
{
  "paths": {
    "local": "/Users/alex.ulyanov/dotfiles/.config.opencode/context",
...
This copies `project-intelligence/` from global (`~/.config/opencode/context/`) to local (`/Users/alex.ulyanov/dotfiles/.config.opencode/context/`), so your project patterns are committed to git and shared with your team. See `/context migrate` for details.

## Common Scenarios

### Scenario 1: Everything Local (Development / Repo Maintainer)
- OAC installed locally via `bash install.sh developer`
- All context in `/Users/alex.ulyanov/dotfiles/.config.opencode/context/`
...
- Run `/add-context` in project → creates `/Users/alex.ulyanov/dotfiles/.config.opencode/context/project-intelligence/` locally
